...
export type FrogConstructorParameters<env extends Env = Env, basePath extends string = '/', _state = env['State']> = Pick<FrameResponse, 'browserLocation'> & Required<Pick<FrameResponse, 'title'>> & {
     ...
     * ```
     */
    initialState?: ((c: Context<env, basePath>) => _state | Promise<_state>) | _state | undefined;
    /**
...
} : method extends 'composerAction' ? ComposerActionOptions | {
    /**
     * Custom handler for Composer Action `GET` response.
     * One can use that if something needs to be derived from the `Context`.
     */
    handler: (c: Context<E, P, I>) => Promise<ComposerActionOptions> | ComposerActionOptions;
} : {});
/**
 * A Frog instance.
 *
...
export declare class FrogBase<env extends Env = Env, schema extends Schema = {}, basePath extends string = '/', _state = env['State']> {
    _initialState: FrogConstructorParameters<env, basePath, _state>['initialState'];
    /** Path for assets. */
    assetsPath: string;
    ...
    headers: FrogConstructorParameters<env, basePath, _state>['headers'] | undefined;
    /** Hono instance. */
    ...
    verifyOrigin: FrogConstructorParameters<env, basePath, _state>['verifyOrigin'];
    metaTags: FrogConstructorParameters<env, basePath, _state>['unstable_metaTags'] | undefined;
