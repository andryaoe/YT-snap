...
import type { CastActionData, CastActionFrameResponseFn, CastActionMessageResponseFn, CastActionResponseFn } from './castAction.js';
import type { ComposerActionData, ComposerActionResponseFn } from './composerAction.js';
...
import type { SignTypedDataResponseFn, SignatureParameters, SignatureResponseFn } from './signature.js';
import type { ContractTransactionResponseFn, SendTransactionParameters, TransactionParameters, TransactionResponseFn } from './transaction.js';
import type { Pretty } from './utils.js';
export type CastActionBaseContext<env extends Env = Env, path extends string = string, input extends Input = {}> = {
    /**
...
export type CastActionContext<env extends Env = Env, path extends string = string, input extends Input = {}> = CastActionBaseContext<env, path, input> & {
...
export type ComposerActionBaseContext<env extends Env = Env, path extends string = string, input extends Input = {}> = {
    /**
     * Data from the action that was passed via the POST body.
...
export type ComposerActionContext<env extends Env = Env, path extends string = string, input extends Input = {}> = ComposerActionBaseContext<env, path, input> & {
    /** Error response that includes message and statusCode. */
    error: BaseErrorResponseFn;
    /**
...
export type FrameBaseContext<env extends Env = Env, path extends string = string, input extends Input = {}, _state = env['State']> = {
    /**
...
export type FrameContext<env extends Env = Env, path extends string = string, input extends Input = {}, _state = env['State']> = FrameBaseContext<env, path, input, _state> & {
    /**
     ...
     * previous frame.
     */
    deriveState: <deriveFn extends (previousState: _state) => void | Promise<void>>(fn?: deriveFn) => ReturnType<deriveFn> extends Promise<void> ? Promise<_state> : _state;
    /** Error response that includes message and statusCode. */
    error: BaseErrorResponseFn;
...
export type TransactionContext<env extends Env = Env, path extends string = string, input extends Input = {}, _state = env['State']> = FrameBaseContext<env, path, input, _state> & {
    /**
...
export type ImageContext<env extends Env = Env, path extends string = string, input extends Input = {}, _state = env['State']> = {
    /**
     * `.env` can get bindings (environment variables, secrets, KV namespaces, D1 database, R2 bucket etc.) in Cloudflare Workers.
     *
     * @example
...
export type SignatureContext<env extends Env = Env, path extends string = string, input extends Input = {}, _state = env['State']> = FrameBaseContext<env, path, input, _state> & {
    /**
